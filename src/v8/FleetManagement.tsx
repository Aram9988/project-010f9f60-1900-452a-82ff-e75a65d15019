...
export default function FleetManagement({ org, app, setApp, currentUser, onNotify }: Props) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {section === "maintenance" && <MaintenanceList requests={maintenance} vehicles={vehicles} org={org} canReply={canManageFleet} onReply={replyMaintenance}/>}
    {section === "requests" && <NeedList requests={needs} vehicles={vehicles} org={org} canReply={canManageFleet} onReply={replyNeed}/>}
...
function MaintenanceList({ requests, vehicles, org, canReply, onReply }: { requests: VehicleMaintenanceRequest[]; vehicles: FleetVehicle[]; org: OrgState; canReply:boolean; onReply:(r:VehicleMaintenanceRequest,s:MaintenanceRequestStatus,date?:string,replacementId?:string,reply?:string)=>void }) {
  return <div className="space-y-3">{requests.length ? requests.map((r)=><MaintenanceCard key={r.id} request={r} vehicles={vehicles} org={org} canReply={canReply} onReply={onReply}/>) : <Empty text="لا توجد طلبات صيانة."/ >}</div>;
}

function MaintenanceCard({ request, vehicles, org, canReply, onReply }: { request: VehicleMaintenanceRequest; vehicles:FleetVehicle[]; org:OrgState; canReply:boolean; onReply:(r:VehicleMaintenanceRequest,s:MaintenanceRequestStatus,date?:string,replacementId?:string,reply?:string)=>void }) {
  ...
  {canReply && <div className="mt-4 flex flex-wrap gap-2">{request.status==="requested" && <><button onClick={()=>onReply(request,"scheduled",date,replacement,reply)} className="gold-action rounded-xl px-3 py-2 text-[9px] font-black"><CalendarDays size={12} className="inline ml-1"/>تحديد الموعد</button><button onClick={()=>onReply(request,"rejected",undefined,undefined,reply)} className="rounded-xl border border-rose-300/15 px-3 py-2 text-[9px] font-black text-rose-300">رفض</button></>}{request.status==="scheduled" && <button onClick={()=>onReply(request,"in_maintenance")} className="rounded-xl border border-amber-300/15 bg-amber-300/5 px-3 py-2 text-[9px] font-black text-amber-300">دخلت الصيانة</button>}{request.status==="in_maintenance" && <button onClick={()=>onReply(request,"completed")} className="rounded-xl border border-emerald-300/15 bg-emerald-300/5 px-3 py-2 text-[9px] font-black text-emerald-300">تمت الصيانة وإعادة المركبة</button>}</div>}
  </section>;
}
...
function NeedList({ requests, vehicles, org, canReply, onReply }: { requests:VehicleNeedRequest[]; vehicles:FleetVehicle[]; org:OrgState; canReply:boolean; onReply:(r:VehicleNeedRequest,s:VehicleNeedStatus,ids:string[],reply:string)=>void }) {
  return <div className="space-y-3">{requests.length ? requests.map((r)=><NeedCard key={r.id} request={r} vehicles={vehicles} org={org} canReply={canReply} onReply={onReply}/>) : <Empty text="لا توجد طلبات مركبات."/ >}</div>;
}

function NeedCard({ request, vehicles, org, canReply, onReply }: { request:VehicleNeedRequest; vehicles:FleetVehicle[]; org:OrgState; canReply:boolean; onReply:(r:VehicleNeedRequest,s:VehicleNeedStatus,ids:string[],reply:string)=>void }) {
  ...
  {canReply && <div className="mt-4 flex flex-wrap gap-2">{canDecision && <><button disabled={!selected.length} onClick={()=>onReply(request,selected.length>=request.quantity?"approved":"partial",selected,reply)} className="gold-action rounded-xl px-3 py-2 text-[9px] font-black disabled:opacity-30"><Check size={12} className="inline ml-1"/>{selected.length>=request.quantity?"موافقة":"موافقة جزئية"}</button><button onClick={()=>onReply(request,"rejected",[],reply)} className="rounded-xl border border-rose-300/15 px-3 py-2 text-[9px] font-black text-rose-300"><CircleX size={12} className="inline ml-1"/>غير متاح</button></>}{["approved","partial"].includes(request.status) && <button onClick={()=>onReply(request,"completed",request.approvedVehicleIds ?? [],reply)} className="rounded-xl border border-emerald-300/15 bg-emerald-300/5 px-3 py-2 text-[9px] font-black text-emerald-300">إنهاء الطلب وإتاحة المركبات</button>}</div>}
  </section>;
}
