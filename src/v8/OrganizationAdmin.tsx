...
export default function OrganizationAdmin({ state, onChange }: { state: OrgState; onChange: (next: OrgState) => void }) {
  ...
  function saveUser(user: OrgUser) {
    ...
    const duplicate = state.users.some((u) => u.id !== user.id && u.username.trim().toLowerCase() === normalizedUsername);
    if (duplicate) {
      window.alert("اسم المستخدم مستخدم بالفعل. اختر اسم مستخدم مختلفاً.");
      return;
    }
    const nextUser = { ...user, name: user.name.trim(), username: user.username.trim() };
    const exists = state.users.some((u) => u.id === user.id);
    onChange({ ...state, users: exists ? state.users.map((u) => u.id === user.id ? nextUser : u) : [...state.users, nextUser] });
    setEditingUser(null);
  }
