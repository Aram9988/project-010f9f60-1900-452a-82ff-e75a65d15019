...
export default function LoginScreen({ org, onLogin }: { org: OrgState; onLogin: (userId: string) => void }) {
  ...
  return <div className="tech-shell login-shell relative grid min-h-screen place-items-center overflow-hidden p-4 text-slate-100"><div className="pointer-events-none absolute inset-0 tech-grid opacity-50" /><div className="login-frame relative z-10 grid w-full max-w-5xl overflow-hidden rounded-[32px] border shadow-2xl backdrop-blur-2xl lg:grid-cols-[1.1fr_.9fr]"><section className="login-brand-panel hidden min-h-[620px] border-l p-10 lg:flex lg:flex-col lg:justify-between"><Brand /><div><div className="text-[10px] font-black tracking-[.2em] text-cyan-300/60">RIF DIMASHQ COMMUNICATIONS</div><h1 className="mt-5 text-4xl font-black leading-[1.4]">مركز المشاريع والمهام والهيكل التنظيمي المباشر</h1><p className="mt-5 max-w-xl text-sm leading-8 text-slate-400">إدارة المشاريع والمهام ومتابعة الأقسام والمكاتب والفريق من شجرة تنظيمية واحدة.</p></div><div className="text-[10px] text-slate-600">OPERATIONS COMMAND CENTER</div></section><section className="login-form-panel flex min-h-[620px] items-center p-6 sm:p-10"><form onSubmit={submit} className="w-full"><div className="mb-8 lg:hidden"><Brand /></div><div className="login-lock-mark grid h-14 w-14 place-items-center rounded-2xl border"><LockKeyhole size={24} /></div><h2 className="mt-6 text-2xl font-black">تسجيل الدخول</h2><p className="mt-2 text-xs leading-6 text-slate-500">أدخل اسم المستخدم وكلمة المرور للوصول إلى مركز المتابعة.</p><div className="mt-7 space-y-4"><Field label="اسم المستخدم"><input className="tech-field" value={username} onChange={(e) => setUsername(e.target.value)} autoComplete="username" /></Field><Field label="كلمة المرور"><input className="tech-field" type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" /></Field></div>{error && <div className="mt-4 rounded-xl border border-rose-400/20 bg-rose-400/7 p-3 text-[11px] font-bold text-rose-300">بيانات الدخول غير صحيحة أو الحساب موقوف.</div>}<button type="submit" className="login-submit mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-2xl text-sm font-black"><ShieldCheck size={17} />دخول</button></form></section></div></div>;
}
