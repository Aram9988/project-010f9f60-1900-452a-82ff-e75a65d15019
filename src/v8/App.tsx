...
function WorkList({ org, currentUser, items, kind, search, status, onSearch, onStatus, onOpen, onCreate }: { org: OrgState; currentUser: OrgUser; items: Assignment[]; kind: WorkType; search: string; status: TaskStatus | "all"; onSearch: (v: string) => void; onStatus: (v: TaskStatus | "all") => void; onOpen: (id: string) => void; onCreate: (k: WorkType) => void }) {
  const list = items.filter((i) => kindOf(i) === kind && (status === "all" || i.status === status) && (!search.trim() || `${i.title} ${i.location ?? ""} ${i.referenceNumber ?? ""}`.toLowerCase().includes(search.trim().toLowerCase()))).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
  ...
  const departmentIssuedItems = list.filter((item) => {
    const issuer = org.users.find((u) => u.id === item.issuedById);
    return roleOf(org, issuer)?.key === "department_head";
  });
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    <div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-black">{kind === "project" ? "المشاريع" : "المهام"}</h1><p className="mt-1 text-[11px] text-slate-500">تم فصل تكليفات رئيس الفرع عن الأعمال التي يوزعها رؤساء الأقسام على فرقهم.</p></div>{canCreate && <button onClick={() => onCreate(kind)} className="flex h-10 items-center gap-2 rounded-xl bg-cyan-300 px-4 text-[11px] font-black text-slate-950"><Plus size={14} />جديد</button>}</div>
...
function WorkListGroup({ title, subtitle, items, org, onOpen, accent }: { title: string; subtitle?: string; items: Assignment[]; org: OrgState; onOpen: (id: string) => void; accent: "command" | "team" | "other" }) {
  const tone = accent === "command" ? "border-cyan-300/15 bg-cyan-300/[0.025]" : accent === "team" ? "border-indigo-300/15 bg-indigo-300/[0.025]" : "border-white/8 bg-white/[0.02]";
  const titleTone = accent === "command" ? "text-cyan-200" : accent === "team" ? "text-indigo-200" : "text-slate-200";
  return <section className={`overflow-hidden rounded-2xl border ${tone}`}><div className="border-b border-white/7 px-5 py-4"><div className="flex items-center justify-between gap-3"><div><h2 className={`text-sm font-black ${titleTone}`}>{title}</h2>{subtitle && <p className="mt-1 text-[9px] leading-5 text-slate-600">{subtitle}</p>}</div><span className="rounded-lg border border-white/7 px-2 py-1 font-mono text-[9px] text-slate-500">{items.length}</span></div></div><div className="divide-y divide-white/7">{items.length ? items.map((i) => { const assignee = org.users.find((u) => u.id === (i.assigneeId ?? i.ownerId)); const issuer = org.users.find((u) => u.id === i.issuedById); return <button key={i.id} onClick={() => onOpen(i.id)} className="flex w-full items-center gap-3 px-5 py-4 text-right hover:bg-white/[0.035]"><div className="min-w-0 flex-1"><div className="truncate text-[15px] font-bold">{i.title}</div><div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-slate-600"><span>المسند إليه: <b className="text-slate-400">{assignee?.name ?? "غير مسند"}</b></span>{issuer && <span>المصدر: <b className="text-slate-400">{issuer.name}</b></span>}{i.location && <span>{i.location}</span>}</div></div><span className="text-[10px] text-slate-600">{statusMeta[i.status].label}</span></button>; }) : <div className="p-8 text-center text-xs text-slate-600">لا توجد أعمال ضمن هذه المجموعة.</div>}</div></section>;
}
