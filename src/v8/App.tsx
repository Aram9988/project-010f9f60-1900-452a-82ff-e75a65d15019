...
function Shell({ org, setOrg, app, setApp, currentUser, onLogout }: { org: OrgState; setOrg: (s: OrgState) => void; app: AppState; setApp: React.Dispatch<React.SetStateAction<AppState>>; currentUser: OrgUser; onLogout: () => void }) {
  ...
  function editUpdate(item: Assignment, updateId: string, text: string) {
    const at = nowIso();
    setApp((s) => ({ ...s, tasks: s.tasks.map((x) => x.id !== item.id ? x : { ...x, updatedAt: at, updates: x.updates.map((u) => u.id !== updateId || u.system ? u : { ...u, originalText: u.originalText ?? u.text, text, editedAt: at, editedById: currentUser.id }) }) }));
    ...
    if (original?.authorId && original.authorId !== currentUser.id) notify(original.authorId, `تم تصحيح أحد تحديثاتك على: ${item.title}`, item.id);
  }
