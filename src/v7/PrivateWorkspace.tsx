...
import { AlertTriangle, BellRing, CalendarDays, Check, ChevronLeft, Circle, Clock3, FileText, NotebookPen, Plus, RotateCcw, Trash2, X } from "lucide-react";
...
export default function PrivateWorkspace({ currentUser }: { currentUser: PrivateWorkspaceUser }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className={`fixed bottom-[92px] left-4 z-[55] flex h-12 items-center gap-2 rounded-2xl border px-4 text-xs font-black shadow-2xl backdrop-blur md:bottom-6 md:left-6 ${overdueCount ? "border-rose-400/35 bg-[#251018]/95 text-rose-200" : "border-cyan-300/20 bg-[#0b1727]/95 text-cyan-200"}`}>
      {overdueCount ? <AlertTriangle size={16} /> : <NotebookPen size={16} />}
      <span className="hidden sm:inline">مساحتي</span>
      {activeReminders.length > 0 && <span className={`grid h-5 min-w-5 place-items-center rounded-full px-1 text-[9px] ${overdueCount ? "bg-rose-400 text-white" : "bg-cyan-300 text-slate-950"}`}>{activeReminders.length}</span>}
    </button>

    {open && <div className="fixed inset-0 z-[100] flex justify-end bg-[#020611]/70 backdrop-blur-sm" onMouseDown={() => setOpen(false)}>
      <section className="h-full w-full max-w-xl overflow-hidden border-r border-white/10 bg-[#081321] shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex h-full flex-col">
          <header className="border-b border-white/8 px-5 py-4">
            <div className="flex items-center justify-between gap-4"><div><div className="text-[9px] font-bold tracking-[.18em] text-cyan-300/45">PRIVATE WORKSPACE</div><h2 className="mt-1 text-lg font-black">المساحة الخاصة</h2><p className="mt-1 text-[10px] text-slate-600">للمدير وقسم الدراسات فقط · {currentUser.label}</p></div><button type="button" onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-xl border border-white/8 text-slate-500 hover:text-white"><X size={16} /></button></div>
            <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl border border-white/8 bg-black/10 p-1"><button type="button" onClick={() => setTab("reminders")} className={`flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${tab === "reminders" ? "bg-cyan-300 text-slate-950" : "text-slate-500"}`}><BellRing size={14} />التذكيرات{overdueCount > 0 && <span className="rounded-full bg-rose-500 px-1.5 py-0.5 text-[8px] text-white">{overdueCount}</span>}</button><button type="button" onClick={() => setTab("notes")} className={`flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${tab === "notes" ? "bg-cyan-300 text-slate-950" : "text-slate-500"}`}><NotebookPen size={14} />الملاحظات</button></div>
          </header>
          <div className="min-h-0 flex-1 overflow-y-auto p-5">{tab === "reminders" ? <Reminders reminders={state.reminders} active={activeReminders} completedCount={completedCount} currentUser={currentUser} onAdd={(reminder) => setState((s) => ({ ...s, reminders: [reminder, ...s.reminders] }))} onComplete={(id) => setState((s) => ({ ...s, reminders: s.reminders.map((r) => r.id === id ? { ...r, completed: true, completedAt: new Date().toISOString() } : r) }))} onRestore={(id) => setState((s) => ({ ...s, reminders: s.reminders.map((r) => r.id === id ? { ...r, completed: false, completedAt: undefined } : r) }))} onDelete={(id) => setState((s) => ({ ...s, reminders: s.reminders.filter((r) => r.id !== id) }))} /> : <Notes notes={state.notes} currentUser={currentUser} onAdd={(note) => setState((s) => ({ ...s, notes: [note, ...s.notes] }))} onUpdate={(id, title, body) => setState((s) => ({ ...s, notes: s.notes.map((n) => n.id === id ? { ...n, title, body, updatedAt: new Date().toISOString() } : n) }))} onDelete={(id) => setState((s) => ({ ...s, notes: s.notes.filter((n) => n.id !== id) }))} />}</div>
        </div>
      </section>
    </div>}
  </>;
...
function Reminders({ reminders, active, completedCount, currentUser, onAdd, onComplete, onRestore, onDelete }: { reminders: Reminder[]; active: Reminder[]; completedCount: number; currentUser: PrivateWorkspaceUser; onAdd: (r: Reminder) => void; onComplete: (id: string) => void; onRestore: (id: string) => void; onDelete: (id: string) => void }) {
  ...
  return <div>
    <div className="flex items-center justify-between gap-3"><div><h3 className="text-base font-black">التذكيرات</h3><p className="mt-1 text-[10px] text-slate-600">المتأخر يتحول للأحمر. استخدم الفلاتر لعرض اليوم أو الأسبوع القادم.</p></div><button type="button" onClick={() => setCreating(true)} className="flex h-9 items-center gap-2 rounded-xl bg-cyan-300 px-3 text-[10px] font-black text-slate-950"><Plus size={13} />تذكير جديد</button></div>

    <div className="mt-4 flex gap-2 overflow-x-auto pb-1">{([['all','الكل'],['overdue',`متأخر ${overdueCount ? `(${overdueCount})` : ''}`],['today','اليوم'],['tomorrow','غداً'],['week','7 أيام'],['later','لاحقاً'],['nodate','بدون موعد']] as [ReminderFilter,string][]).map(([key,label]) => <button key={key} type="button" onClick={() => setFilter(key)} className={`shrink-0 rounded-xl border px-3 py-2 text-[9px] font-black ${filter === key ? key === 'overdue' ? 'border-rose-400/30 bg-rose-400/10 text-rose-200' : 'border-cyan-300/25 bg-cyan-300/8 text-cyan-200' : 'border-white/7 text-slate-600 hover:text-slate-400'}`}>{label}</button>)}</div>
    ...
    <div className="mt-5 overflow-hidden rounded-2xl border border-white/8 bg-white/[0.02]">{visible.length ? <div className="divide-y divide-white/7">{visible.map((r) => { const overdue = isOverdue(r); return <div key={r.id} className={`group flex items-start gap-3 px-4 py-4 ${overdue ? 'bg-rose-500/[0.075]' : ''}`}><button type="button" onClick={() => onComplete(r.id)} aria-label="إنهاء التذكير" className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full border-2 text-transparent transition ${overdue ? 'border-rose-400/70 hover:bg-rose-400' : 'border-cyan-300/45 hover:bg-cyan-300'} hover:text-slate-950`}><Check size={13} /></button><div className="min-w-0 flex-1"><div className={`text-sm font-bold leading-6 ${overdue ? 'text-rose-100' : 'text-slate-100'}`}>{r.text}</div><div className={`mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] ${overdue ? 'text-rose-300/75' : 'text-slate-600'}`}><span>إلى: <b className={overdue ? 'text-rose-200' : 'text-slate-400'}>{r.forWhom}</b></span><span className="inline-flex items-center gap-1"><Clock3 size={10} />{formatDue(r.dueAt)}</span>{overdue && <span className="inline-flex items-center gap-1 rounded-full bg-rose-500/15 px-2 py-0.5 font-black text-rose-300"><AlertTriangle size={10} />متأخر</span>}</div></div><button type="button" onClick={() => onDelete(r.id)} className="mt-1 opacity-0 text-slate-700 transition group-hover:opacity-100 hover:text-rose-300"><Trash2 size={13} /></button></div>; })}</div> : <div className="px-5 py-12 text-center"><CalendarDays size={28} className="mx-auto text-slate-700" /><div className="mt-3 text-xs font-bold text-slate-500">لا توجد تذكيرات ضمن هذا العرض</div><div className="mt-1 text-[10px] text-slate-700">جرّب تغيير الفلتر أو أضف تذكيراً جديداً.</div></div>}</div>
...
function Notes({ notes, currentUser, onAdd, onUpdate, onDelete }: { notes: Note[]; currentUser: PrivateWorkspaceUser; onAdd: (n: Note) => void; onUpdate: (id: string, title: string, body: string) => void; onDelete: (id: string) => void }) {
  ...
  return <div>
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2">{notes.length ? notes.map((note) => <div key={note.id} role="button" tabIndex={0} onClick={() => edit(note)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') edit(note); }} className="group min-h-40 cursor-pointer rounded-2xl border border-white/8 bg-white/[0.025] p-4 text-right transition hover:border-cyan-300/15 hover:bg-white/[0.04]"><div className="flex items-start justify-between gap-3"><FileText size={15} className="mt-0.5 shrink-0 text-cyan-300/70" /><button type="button" onClick={(e) => { e.stopPropagation(); onDelete(note.id); }} className="opacity-0 text-slate-700 transition group-hover:opacity-100 hover:text-rose-300"><Trash2 size={13} /></button></div><h4 className="mt-3 line-clamp-2 text-sm font-black leading-6">{note.title}</h4><p className="mt-2 line-clamp-3 whitespace-pre-wrap text-[11px] leading-6 text-slate-500">{note.body || "ملاحظة بدون نص"}</p><div className="mt-4 text-[9px] text-slate-700">آخر تعديل {formatDate(note.updatedAt)}</div></div>) : <div className="col-span-full py-12 text-center"><NotebookPen size={30} className="mx-auto text-slate-700" /><div className="mt-3 text-xs font-bold text-slate-500">لا توجد ملاحظات بعد</div></div>}</div>
  </div>;
}
