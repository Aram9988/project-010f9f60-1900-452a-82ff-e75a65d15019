...
function TaskNameRow({ task, onOpen }: { task: Assignment; onOpen: (id: string) => void }) {
  return <button type="button" onClick={() => onOpen(task.id)} className="block w-full px-5 py-4 text-right transition hover:bg-white/[0.035]"><div className="truncate text-[15px] font-bold leading-7 text-slate-100">{task.title}</div></button>;
}
...
function Tasks({ tasks, search, filter, onSearch, onFilter, onOpen, onCreate, canCreate }: { tasks: Assignment[]; search: string; filter: TaskStatus | "all" | "attention"; onSearch: (v: string) => void; onFilter: (v: TaskStatus | "all" | "attention") => void; onOpen: (id: string) => void; onCreate: () => void; canCreate: boolean }) {
  const list = useMemo(() => tasks.filter((t) => { const q = search.trim().toLowerCase(); return (!q || `${t.number} ${t.title} ${t.details}`.toLowerCase().includes(q)) && (filter === "all" || (filter === "attention" ? ["waiting","returned"].includes(t.status) : t.status === filter)); }).sort((a,b) => b.updatedAt.localeCompare(a.updatedAt)), [tasks,search,filter]);
  ...
  return <div className="mx-auto max-w-4xl space-y-5"><div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-black">التكليفات</h1><p className="mt-1 text-[11px] text-slate-500">اختر التكليف لعرض التفاصيل.</p></div>{canCreate && <button onClick={onCreate} className="flex h-10 shrink-0 items-center gap-2 rounded-xl bg-cyan-300 px-4 text-[11px] font-black text-slate-950"><Plus size={15} />تكليف جديد</button>}</div><div className="space-y-3"><div className="relative"><Search size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-600" /><input value={search} onChange={(e) => onSearch(e.target.value)} placeholder="ابحث عن تكليف..." className="h-12 w-full rounded-2xl border border-white/8 bg-white/[0.025] pr-11 pl-4 text-sm outline-none placeholder:text-slate-600 focus:border-cyan-300/20" /></div><div className="flex gap-2 overflow-x-auto pb-1">{filters.map(([k,l]) => <button key={k} onClick={() => onFilter(k)} className={`shrink-0 rounded-lg px-3 py-1.5 text-[10px] font-bold transition ${filter === k ? "bg-cyan-300 text-slate-950" : "text-slate-500 hover:bg-white/5 hover:text-slate-300"}`}>{l}</button>)}</div></div><div className="overflow-hidden rounded-2xl border border-white/8 bg-white/[0.02]"><div className="divide-y divide-white/7">{list.length ? list.map((t) => <TaskNameRow key={t.id} task={t} onOpen={onOpen} />) : <div className="p-10 text-center text-xs text-slate-600">لا توجد تكليفات مطابقة.</div>}</div></div></div>;
}
