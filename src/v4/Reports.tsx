...
export default function Reports({ tasks, user }: { tasks: Assignment[]; user: DemoUser }) {
  ...
  const rows = useMemo(() => tasks.filter((item) => {
    ...
    return deptOk && workOk && typeOk;
  }).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)), [tasks, range, departmentId, workType, type]);

  function exportExcel() {
    const data = rows.map((item) => ({
      "النوع": kindLabel(item),
      "الرقم": item.number,
      "الاسم": item.title,
      "القسم": deptName(item.departmentId),
      "الحالة": statusMeta[item.status].label,
      "الأولوية": priorityMeta[item.priority],
      "آخر تحديث": fmt(item.updatedAt),
    }));
  ...
  function exportCsv() {
    const table = [["النوع", "الرقم", "الاسم", "القسم", "الحالة", "الأولوية", "آخر تحديث"], ...rows.map((item) => [kindLabel(item), item.number, item.title, deptName(item.departmentId), statusMeta[item.status].label, priorityMeta[item.priority], fmt(item.updatedAt)])];
